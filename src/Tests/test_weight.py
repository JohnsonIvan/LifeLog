...
BATCH_GET_ALL_PARAMS = {
    "since": BATCH_GET_ALL_SINCE,
    "before": BATCH_GET_ALL_BEFORE,
    "limit": 2 * BATCH_GET_ALL_COUNT + 100,
    "offset": 0,
}
...
def test_get_missingParam(client):
    for key in ["since", "before", "limit", "offset"]:
        ...
        ), f'key = "{key}"; url = "{url}"; status = {status}'
...
def test_entry_add_happy(monkeypatch, client):
    ...
    params = urllib.parse.urlencode(
        {"weight": value_units, "datetime": 450, "units": units}
    )
    ...
    assert len(data) == 6
    list_truths = list(
        map(lambda x: fuzzy_equals([None, "450", str(value_kg)], x), data)
    )
...
@pytest.mark.unit
@pytest.mark.parametrize(
    "expected_code, params_dict, conv_ret",
    [
        (
...
def test_entry_add_auth(client, monkeypatch):
    ...
    params = urllib.parse.urlencode(
        {"weight": weight, "datetime": datetime, "units": units}
    )
...
def test_entry_add_autocommits(app, client, monkeypatch):
    ...
    params = urllib.parse.urlencode(
        {"weight": 0.1, "datetime": 450, "units": "kilograms"}
    )
    url = ENTRY_URL + "?" + params

    test_db.count_commits(
        app,
        client.post,
        url,
        monkeypatch,
...
def test_entry_delete_happy(app, client):
    ...
    with app.app_context():
        ...
        results = db.execute(
            "SELECT * FROM weight WHERE userid=? AND id=?",
            (auth_tests.AUTH_USERID, entry_id),
        ).fetchall()
        assert len(results) == 1

        response = client.delete(
            ENTRY_URL + f"/{entry_id}", headers=auth_tests.AUTH_HEADERS
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            0.87282,
            "148064f1-48bc-415d-9ff8-8a57d7ad8687",
        ),
        (
            True,
            True,
...
def test_entry_update(
    ...
    with app.app_context():
        ...
        response = client.put(
            ENTRY_URL + f"/{entry_id}?{sParams}", headers=auth_tests.AUTH_HEADERS
        )
...
@pytest.mark.unit
@pytest.mark.parametrize(
    "input_value, input_unit, expected, max_error",
    [
