...
sudo install -D "--mode=${CHMOD_PERMISSIONS}"             "dist/$APP_NAME-"*".whl"                    "$CODE_DIR" # TODO: is it possible for there to be multiple versions?
sudo install -D "--mode=${CHMOD_PERMISSIONS}"             "$SCRIPTS_DIR/launch.bash"                  "$CODE_DIR"
sudo install -D "--mode=${CHMOD_PERMISSIONS}"             "$SCRIPTS_DIR/lifelogserver.service"        "/etc/systemd/system/"
sudo install -D "--mode=${CHMOD_PERMISSIONS}"             "$SCRIPTS_DIR/net.ivanjohnson.lifelog.conf" "/etc/nginx/servers/"
...
Purge/reinitialize the database if necessary (automatic migrations have not yet been implemented, as of 2022-09-19):
	...
	Restore backup:
		# UNTESTED
		sudo install --owner "$USERNAME" --group "$USERNAME" -d ./path/to/backup/${PROD_ENV_DIR_NAME}_var_LifeLogServer-instance.bak -t "${CODE_DIR}/${PROD_ENV_DIR_NAME}/var/LifeLogServer-instance"
	(Re-)initialize:
		sudo -u "$USERNAME" bash -c 'cd "$CODE_DIR"; source ./${PROD_ENV_DIR_NAME}/bin/activate; export FLASK_APP=LifeLogServer; flask init-db'
